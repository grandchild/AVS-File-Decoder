...
var getUInt = function (blob, offset, size) {
    ...
    switch (size) {
        case 1:
            return blob[offset];
        case sizeInt:
            return _this.getUInt32(blob, offset);
        case sizeInt * 2:
            return _this.getUInt64(blob, offset);
        default:
...
var getBool = function (blob, offset, size) {
    var val = size === 1 ? blob[offset] : _this.getUInt32(blob, offset);
    return [val !== 0, size];
};
...
var getSizeString = function (blob, offset, size) {
    ...
    var getHidden = false;
    if (!size) {
        size = _this.getUInt32(blob, offset);
        add = sizeInt;
    }
    else {
