...
export default {
		...
		generic(blob: Uint8Array, offset: number, fields: unknown, name: string, group: string, end: number): unknown {
				...
				for (let i = 0; i < keys.length; i++) {
						...
						let size = 0;
						let value: Webvsc.JSONPrimitive;
						let result: [Webvsc.JSONPrimitive, number];
		...
		// eslint-disable-next-line @typescript-eslint/no-unused-vars
		versioned_generic(blob: Uint8Array, offset: number, fields: Webvsc.JSONObject, name: string, group: string, end: number): unknown {
